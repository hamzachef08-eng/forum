CREATE DATABASE IF NOT EXISTS est_forum
...
CREATE TABLE modules (
  ...
  PRIMARY KEY (id),
  KEY idx_modules_choice (filiere, semestre)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE module_messages (
  ...
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
  KEY idx_mm_module (module_id),
  KEY idx_mm_user (user_id),
  KEY idx_mm_parent (parent_message_id),
...
CREATE TABLE message_reports (
  ...
  status ENUM('OPEN','BANNED','DISMISSED') NOT NULL DEFAULT 'OPEN',
  reviewed_by BIGINT UNSIGNED NULL,
  reviewed_at DATETIME NULL,
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
  KEY idx_rep_status (status),
  KEY idx_rep_message (message_id),
...
nom = VALUES(nom), prenom = VALUES(prenom), mot_de_passe = VALUES(mot_de_passe), filiere = VALUES(filiere), semestre = VALUES(semestre), role = VALUES(role), is_banned = VALUES(is_banned), email_verified = VALUES(email_verified);
...
ON DUPLICATE KEY UPDATE
nom = VALUES(nom), prenom = VALUES(prenom), mot_de_passe = VALUES(mot_de_passe), role = VALUES(role), is_banned = VALUES(is_banned), email_verified = VALUES(email_verified);
